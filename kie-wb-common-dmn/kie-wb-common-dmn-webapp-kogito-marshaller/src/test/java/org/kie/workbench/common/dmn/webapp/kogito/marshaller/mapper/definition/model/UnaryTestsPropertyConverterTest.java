...
public class UnaryTestsPropertyConverterTest {
    ...
    @Mock
    private JSITUnaryTests dmnUnary;
    ...
    public void setup() {
        ...
        when(dmnUnary.getOtherAttributes()).thenReturn(additionalAttributes);
    ...
    public void testDmnFromWB(final ConstraintType constraintType) {
        ...
        final JSITUnaryTests unaryTests = UnaryTestsPropertyConverter.dmnFromWB(wbUnaryTests);
        assertEquals(ID, unaryTests.getId());
        assertEquals(TEXT, unaryTests.getText());
        assertEquals(constraintType.value(), unaryTests.getOtherAttributes().get(CONSTRAINT_KEY));
    }
}