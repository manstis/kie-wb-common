...
@Ignore("Lots of work to do!")
@RunWith(MockitoJUnitRunner.class)
public class DMNMarshallerKogitoTest {

    private static final Logger LOG = LoggerFactory.getLogger(DMNMarshallerKogitoTest.class);
...
//        DMNMarshallerKogito m = getDMNMarshaller();
...
//
//        DecisionTable dmnDT = (DecisionTable) model.getDecisionByName("my decision").getDecision().getExpression();
...
//        final DMNMarshallerKogito m = getDMNMarshaller();
//        @SuppressWarnings("unchecked")
...
//                                      Decision.class);
//
//        Node<?, ?> rootNode = DMNMarshallerKogito.findDMNDiagramRoot((Graph) g);
...
//                   messages0.stream().filter(m -> m.getLevel().equals(Level.ERROR)).count() > 0);
//
//        DMNMarshallerKogito m = getDMNMarshaller();
//
//        // first unmarshal from DMN XML to Stunner DMN Graph
...
//        assertTrue("The DMN XML content did NOT result in compilation errors and this test method expected errors to be detected. If this was intentional use test method roundTripUnmarshalMarshalThenUnmarshalDMN",
//                   messages0.stream().filter(msg -> msg.getLevel().equals(Level.ERROR)).count() > 0);
//
...
//                                      Decision.class);
//
//        final Node<?, ?> rootNode = DMNMarshallerKogito.findDMNDiagramRoot((Graph) g);
//        assertNotNull(rootNode);
//        assertRootNodeConnectedTo(rootNode,
//                                  decisionNode);
//        assertEquals("decisionNode parent is Definitions DMN root",
...
//        DMNMarshallerKogito marshaller = getDMNMarshaller();
//        DiagramImpl diagram = createDiagram();
...
//        final DMNMarshallerKogito marshaller = getDMNMarshaller();
//
...
//        Node diagramRoot = DMNMarshallerKogito.findDMNDiagramRoot(g);
...
//
//        DMNMarshallerKogito.connectRootWithChild(diagramRoot, inputDataNode);
//        DMNMarshallerKogito.connectRootWithChild(diagramRoot, decisionNode);
//
//        DMNMarshallerKogito m = getDMNMarshaller();
...
//        Edge myEdge = api.getFactoryManager().newElement(edgeID, DMNMarshallerKogito.ASSOCIATION_ID).asEdge();
...
//
//        DMNMarshallerKogito.connectRootWithChild(diagramRoot, inputDataNode);
//        DMNMarshallerKogito.connectRootWithChild(diagramRoot, textAnnotationNode);
//
//        DMNMarshallerKogito m = getDMNMarshaller();
//        String output = m.marshall(diagram);
//        LOG.debug(output);
...
//
//        final DMNMarshallerKogito marshaller = spy(getDMNMarshaller());
//        final List<DRGElement> importedDRGElements = mock(List.class);
//        final Map<Import, Definitions> importDefinitions = mock(Map.class);
//        final Definitions dmnXml = mock(Definitions.class);
//
//        final DRGElement ref1 = mock(DRGElement.class);
//        final DRGElement ref2 = mock(DRGElement.class);
//        final DRGElement ref3 = mock(DRGElement.class);
//
...
//
//        final DMNMarshallerKogito marshaller = spy(getDMNMarshaller());
//        final List<DRGElement> importedDrgElements = new ArrayList<>();
//
//        final DRGElement imported = mock(DRGElement.class);
...
//    }
//
//    private DRGElement createDRGElementWithNamespaceAndId(final String namespace,
//                                                          final String id) {
//
//        final DRGElement drgElement = mock(DRGElement.class);
...
//        final List<ItemDefinition> actualItemDefinitions = new ArrayList<>();
//        final Map<Import, Definitions> importDefinitions = new HashMap<>();
//        final DMNMarshallerKogito dmnMarshaller = spy(getDMNMarshaller());
//
...
//        final List<ItemDefinition> actualItemDefinitions = new ArrayList<>(asList(itemDefinition1, itemDefinition2, itemDefinition3));
//        final List<ItemDefinition> expectedItemDefinitions = asList(itemDefinition1, itemDefinition3);
//        final DMNMarshallerKogito dmnMarshaller = getDMNMarshaller();
//
...
//        final List<DRGElement> drgElements = new ArrayList<>(Arrays.asList(e1, e2, e3));
...
//        for (Namespace nsp : Namespace.values()) {
...
//                for (Entry<String, String> pair : pfxAndURI) {
...
//    }
//
//    private DMNMarshallerKogito getDMNMarshaller() {
//        return new DMNMarshallerKogito(api.getFactoryManager(),
